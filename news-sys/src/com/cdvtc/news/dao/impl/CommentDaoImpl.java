...
public class CommentDaoImpl implements CommentDao {
    ...
    public List<Comment> getCommentsByNewsId(int newsId) {
        ...
        try {
            ...
            //执行查询并返回结果集
            PreparedStatement  pst = con.prepareStatement("select c.*, r.id as rid, u.nickname, u.photo from `comment` as c left join `comment` as r on r.reply_for_id = c.id left join `user` as u on c.creator=u.id  where c.news_id=?");
            pst.setInt(1, newsId);
            ...
            while (rs.next()) {
                ...
                comment.setHidden(rs.getBoolean("hidden"));
    ...
    public void hiddenComment(long commentId, boolean hidden) {
        ...
        //执行查询并返回结果集
        try {
            PreparedStatement  pst = con.prepareStatement("update comment set hidden=? where id=?");
            pst.setBoolean(1, hidden);
            pst.setLong(2, commentId);
