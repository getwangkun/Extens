    ...
    <div class="media-body">
        ...
        <p>
            <c:if test="${comment.hidden}">
                <span class="text-muted"><i>评论内容已删除</i></span>
            </c:if>
            <c:if test="${!comment.hidden}">
                ${comment.content}
            </c:if>
        ...
        <p id="comment-${comment.id}">
           <span class="text-info"><%=TimeFormat.getInterval(comment.getPubDate())%></span>
            <c:if test="${!comment.hidden}">
                <span class="pull-right"><button class="btn-link btn-reply" onclick="reply(${comment.id})"
                                                 id="btn-${comment.id}">回复</button></span>
                <c:if test="${comment.creator.id == sessionScope.user.id}">
                    <span class="pull-right"><button class="btn-link" onclick="deleteComment(${comment.id})">删除</button></span>
                </c:if>
            </c:if>
        </p>
        <hr>
